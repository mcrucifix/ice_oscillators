...
Xp = function (t,y,parms) {  
 dx =  - ( y[2] ) #  + 0.50* forcing(t)
 dy =  - parms$epsilon*(phi(1.0 * y[2]) - y[1])  
 dz =  -  10*(y[3] - y[2]  )
...
pdf('Nominal-3well.pdf', 5, 5, pointsize=11)
plot(times, Sol[,2], type='l', frame=FALSE, axes=FALSE, ylim=c(-4,1), xlim=c(5,15), lwd=2, xlab='times (Arb. Units)', ylab='', title='nominal model but with 3-well potential surface')
lines(times, Sol[,2], type='l')
lines(times, Sol[,3]-2.5, type='l', lwd=2)

mtext('x', side=2, at=0, line=3)
mtext('y', side=2, at=-2.5, line=3)

axis(2, at=seq(-2,2)/2,    labels=seq(-2,2)/2.)
axis(2, at=seq(-2,2)/2-2.5,  labels=seq(-2,2)/2.)
