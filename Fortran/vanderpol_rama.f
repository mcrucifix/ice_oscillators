...
! \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
      ...
      subroutine read_insol(na,ampsin,ampcos,ome)
       ...
       parameter (scale = 1d4 / 1.532497854)

      open (10, file='insol65nber78.dat'   ,status='old')
       read (10,*)  ! skip first line
       do i=1,na
         read (10,*) tmp1,tmp2,tmp3
!        ome(i)=tmp1*scale * 2.47
         ome(i)= 2.47 
         ampsin(i)=0.
         ampcos(i)=1.
       enddo
...
!                    (if not known : may be obtained by spin-up)
     ...
     c                  status='unknown')
      ...
      DO m=1,N
        ...
        da(1)  =     DYT
        da(2)  =    - ALPHA * ((1 - X*X)*DYT - 2*X*Y*DXT) - DXT
